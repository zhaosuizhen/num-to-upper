...
export default {
  ...
  methods:{
    moneyToCN(){
        ...
        // 现在是字符串类型！！
        let integer = arrMoney[0]
        let decimal = arrMoney[1]
        
        // 整数部分的长度
        let len = integer.length

        // 大数位的个数
        let pitch = Math.ceil(len / 4)
        
        // 定义空数组用来存放每一个大数位
        let arrInt = []

        // 将整数部分从各位开始，每四个分为一个数位，存到数组中
        for (let index = 0; index < pitch; index++) {// 有几个数位就循环几次
          arrInt.push(
                      !index?// 判断是否为第一次循环，
                      integer.slice(0 , len % 4 == 0? 4 : len % 4)://如果是第一次循环，就截取最大的数位
                      integer.slice( 
                                     ( (len % 4 == 0? 4 : len % 4) + (index-1) * 4) ,
                                     ( (len % 4 == 0? 4 : len % 4) + (index) * 4) 
        ...
        let arr1 = arrInt.map((item, index) => {
          
          // 将arrInt数组中每一个成员转成汉字格式
          let arr2 = item.split('').map((val, key) => {// 先将字符串传为数组，遍历转换
          // 如果当前成员是字符串“0”，只把他换成汉字“零”。
          //如果不是“0”，把他转成对应的汉字后再加上小数位的单位
          // 例如['一千','两百','三十','四']
            return val === '0' ? numCN[Number(val)] : (numCN[Number(val)] + SMALL[item.length - key - 1])
          ...
          return arr2.length ?  //判断当前项是否为空
                 (arr2 + BIG[pitch - 1 - index]) : //不为空的话，给当前项加上对应的大数位
        ...
        // 3.将所有连着的“零”，换成一个“零”
        // 4.将“一十”改为“十”
        let resultInt = arr1.join('').replace(/^\u96f6+/, '')
                               .replace(/\u96f6+$/, '')
                               .replace(/\u96f6+/g, '\u96f6')
                               .replace(/^\u4e00\u5341/, '\u5341')
...
/***************************************整数部分处理完成***************************************/
        ...
        if (!decimal || decimal === '0' || decimal === '00') {
          ...
          let [p0, p1] = decimal.split('')
          decimalCN = (p0 === '0' ? '零' : (numCN[Number(p0)] + '角')) + 
                      (!p1 || p1 === '0' ? '' : (numCN[Number(p1)] + '分'))//如果没有分，或者分为“0”的时候为空，
        }
/***************************************小数部分处理完成***************************************/

        // 如果整数部分有值的话就用整数部分,没有整数部分用“零”
        return `${resultInt || numCN[0]}元${decimalCN}`
    }
